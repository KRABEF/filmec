...
export function ThemeToggle() {
  ...
  return (
    <div className="flex items-center gap-3 select-none">
      ...
      <button
        onClick={toggleTheme}
        className={`relative w-20 h-10 flex items-center rounded-full transition-colors duration-300 
        ${theme === "light" ? "bg-gray-200" : "bg-neutral-800"}`}
      >
        <div
          className={`absolute top-1 left-1 w-8 h-8 flex items-center justify-center 
          rounded-full bg-amber-600 text-white transition-all duration-300
